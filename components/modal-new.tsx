...
import { ChevronsUpDown, Edit3, Folder, FolderPlus, Notebook, Plus, TerminalSquare, X } from "lucide-react";
import { toast } from "sonner";
import { useNavigate } from "react-router-dom";
import { Prompt, ResolvedFolder, SaveFileController } from "@/utils/types";
import { Button } from "./ui/button";
import { DialogHeader, DialogFooter, DialogPortal, Dialog, DialogContent, DialogTitle, DialogClose } from "./ui/dialog";
...
export interface NewModalRef {
  promptUser: (preset?: "chat" | "prompt" | undefined, hideChatTab?: boolean) => Promise<void>;
}
...
const NewModal = forwardRef<NewModalRef, NewModalProps>(({ folders, controller }, ref) => {
  // IS THIS NEEDED?
  ...
  const invalid = selectedTab === "chat" ? chatNameInvalid : promptNameInvalid || newFolderNameInvalid;
  ...
  useImperativeHandle(ref, () => ({
    promptUser: (preset?: "chat" | "prompt" | undefined, hideChatTab = false) => {
      reset();
      setIsOpen(true);
      if (preset) {
  ...
  const handleCreate = useCallback(() => {
    if (selectedTab === "chat") {
      ...
      const { newId, error } = controller.prompts.add(promptName, folderId, promptType);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      ...
      <DialogContent className="max-w-[350px] pt-4">
        <DialogHeader>
          <DialogTitle className="flex items-center">{hideChatTab ? "Create new prompt" : "Create new"}</DialogTitle>
        </DialogHeader>
        <Tabs value={selectedTab} onValueChange={(v) => setSelectedTab(v as "chat" | "prompt")} className="min-w-0 flex-1">
          ...
          <TabsContent value="chat" className="mt-4 flex flex-col gap-4">
            ...
            <Input
              ...
              className={chatNameInvalid ? "border-red-500 focus-visible:ring-red-500" : ""}
            />
          </TabsContent>
          ...
          <TabsContent value="prompt" className="flex flex-col gap-4">
            ...
            <Input
              ...
              className={promptNameInvalid ? "border-red-500 focus-visible:ring-red-500" : ""}
            />
            <Label className="flex items-center">
            ...
            {isNewFolder ? (
              <div className="flex gap-2">
                <Input
                  ...
                  className={newFolderNameInvalid ? "border-red-500 focus-visible:ring-red-500" : ""}
                />
                {canCancelNewFolder && (
                  <Button
                    variant="secondary"
              ...
              <Select value={selectedFolderId} onValueChange={handleFolderChange}>
                ...
                <SelectContent className="max-w-[300px]">
                  <div className="max-h-[200px] overflow-y-auto">
                    {folders.map((folder) => (
                      <SelectItem key={folder.id} value={folder.id} className="block">
