...
export default function PromptPage({
  ...
  const handleFolderSelect = useCallback(async () => {
    ...
    if ('folderId' in res) {
      ...
      const { error } = controller.prompts.changeFolder(
        prompt.id,
        res.folderId,
      );
      ...
      const { error: changeFolderError } = controller.prompts.changeFolder(
        prompt.id,
        newId,
      );
        ...
        <div className="flex items-center gap-2 px-4 text-sm text-muted-foreground @container">
          ...
          >
            ...
            <span className="truncate">
              {prompt.folder ? prompt.folder.name : 'No folder'}
            </span>
          </Button>
          <span>
            <Separator
          ...
          <Select
            value={prompt.type}
            onValueChange={(v) => handleSetType(v as ResolvedPrompt['type'])}
          >
